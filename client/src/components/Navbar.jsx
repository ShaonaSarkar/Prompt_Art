...
const Navbar = () => {
  ...
  return (
    <div className='flex items-center justify-between py-4'>
        <Link to='/'><img src = {assets.logo} alt="The logo of the website" className='w-28 sm:w-32 md:w-36 lg:w-40 '  />
        </Link>
        <div>
            {user ? 
            <div className='flex items-center gap-2 sm:gap-3'>
                ...
                <p className='text-gray-600 max-sm:hidden pl-3'>Hi, {user.name}</p>
                <div className='relative group'>
                    ...
                    <div className='absolute hidden group-hover:block top-0 right-0 z-10 text-black pt-12'>
                        <ul className='list-none m-0 p-2 bg-white rounded-md text-sm border border-neutral-200'>
            ...
            <div className='flex items-center gap-2 sm:gap-5'>
                ...
                <button onClick = {() => setShowLogin(true)}className='bg-zinc-950 text-white py-2 px-7 sm:px-10 text-sm rounded-full cursor-pointer'>Login</button>
            </div>
            }
        </div>
    </div>
